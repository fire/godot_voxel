...
bool can_split(Vector3i node_origin, int node_size, const VoxelAccess &voxels, float geometric_error) {
	...
	Vector3i center_pos = node_origin + Vector3i(node_size / 2, node_size / 2, node_size / 2);
	...
	for (int i = 0; i < 19; ++i) {
		...
		float interpolated_value = positions_ratio[i].interpolate(v0, v1, v2, v3, v4, v5, v6, v7);
	...
	if (is_border_back(n0) && is_border_back(n1) && is_border_back(n4) && is_border_back(n5)) {
		...
		if (is_border_bottom(n0) && is_border_bottom(n1)) {
			...
			if (is_border_left(n0)) {
				add_cell(grid, Vector3(n0->origin), get_center_back_bottom(n0), get_center_bottom(n0), get_center_left_bottom(n0),
...
void polygonize_dual_cell(const DualCell &cell, const VoxelAccess &voxels, MeshBuilder &mesh_builder, bool skirts_enabled) {
	...
	if (skirts_enabled) {
		add_marching_squares_skirts(corners, values, mesh_builder, Vector3(), Vector3(voxels.buffer.get_size() + voxels.offset));
	}
}
...
void VoxelMesherDMC::build(VoxelMesher::Output &output, const VoxelMesher::Input &input) {
	...
	if (root != nullptr) {
		...
		dmc::polygonize_volume_directly(
				voxels, Vector3i(PADDING, PADDING, PADDING), Vector3i(chunk_size, chunk_size, chunk_size), cache.mesh_builder, skirts_enabled);
		stats.meshing_time = OS::get_singleton()->get_ticks_usec() - time_before;
	}
